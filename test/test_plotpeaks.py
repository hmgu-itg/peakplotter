...
def test_read_asoc_for_NA_values_in_extra_column():
    """
    summary statistics file with NA values
    This is to check whether 'NA' values in an extra column causes the read_assoc function to fail in reading the file.
    Output is identical to input.
    """
    chr_col, pos_col, pval_col, maf_col, rs_col, a1_col, a2_col, extra = 'c', 'pos', 'pval', 'maf', 'rs', 'a1', 'a2','extra'
    example = pd.DataFrame([
            [1, 'rs001', 100, 'G', 'T', 0.001, 0.01, 'NA'],
            [1, 'rs002', 200, 'G', 'T', 0.01, 0.01, 'NA']
        ]
    ...
    example_buff.seek(0)
    logger = get_test_logger()
    iterable = plotpeaks.read_assoc(example_buff, chr_col, pos_col, pval_col, maf_col, rs_col, a1_col, a2_col, extra, logger)
    data = next(iterable)
    expected = pd.DataFrame([
