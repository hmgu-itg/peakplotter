...
sub Protein {
    ...
    foreach my $variant (keys %variants){
        ...
        my $URL = sprintf("http://grch37.rest.ensembl.org/overlap/id/%s?feature=CDS", $geneID);
...
sub _getUniPortID {
    ...
    my $URL = sprintf("http://grch37.rest.ensembl.org/xrefs/id/%s", $ProtID);
    my $http = HTTP::Tiny->new();
    my $UNIPROT_id = "-";
    ...
    $URL = sprintf("http://grch37.rest.ensembl.org/overlap/translation/%s", $ProtID);
    $http = HTTP::Tiny->new();
    my $Transcript_id = "-";
