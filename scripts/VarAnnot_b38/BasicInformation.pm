...
sub pos_finder_rs {
    ...
    my $rsid = $hash{"rsID"};
    my $URL = sprintf("http://rest.ensembl.org/variation/human/%s?content-type=application/json", $rsid);
...
sub rs_finder {
    ...
    my $URL = '';
    if ($end < $start) {
        $URL = sprintf("http://rest.ensembl.org/overlap/region/human/%s\:%s-%s?feature=variation", $chr, $end, $start);
    }
    else {
        $URL = sprintf("http://rest.ensembl.org/overlap/region/human/%s\:%s-%s?feature=variation", $chr, $start, $end);
    }
